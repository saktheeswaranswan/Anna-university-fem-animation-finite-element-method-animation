...
function draw() {
  ...
  drawImages(matrixA, 0, 0);
  drawImages(matrixB, 0, 300);
  drawImages(matrixC, 0, 600);
  drawImages(matrixD, 0, 900);
}
...
function drawImages(matrix, offsetX, offsetY) {
  for (let r = 0; r < 6; r++) {
    for (let c = 0; c < 6; c++) {
      let img = r === c ? happyImg : sadImg;
